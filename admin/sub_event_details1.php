...
<head>
    ...
    <link rel="stylesheet" href="path/to/your/bootstrap.css"> <!-- Make sure to link to your Bootstrap CSS -->
    <script src="path/to/your/jquery.min.js"></script> <!-- Make sure to link to jQuery -->
    <script>
        $(document).ready(function(){
            let contestantCount = 2; // Initial count of contestants

            // Function to add a new contestant form
            function addContestant() {
                contestantCount++;
                const newContestant = `
                    <p class="text-box">
                        <label for="box${contestantCount}">Contestant No. <span class="box-number">${contestantCount}</span></label> <br />
                        <input type="file" name="photo${contestantCount}" required />
                        <input type="text" placeholder="Contestant Fullname" name="con${contestantCount}" id="box${contestantCount}" required />
                        <input type="text" placeholder="Addon" class="addon" name="addon${contestantCount}" id="addon${contestantCount}" style="display:none;" />
                        <input type="hidden" value="${Math.floor(Math.random() * (999999 - 100000 + 1)) + 100000}" name="rand${contestantCount}" required />
                        <br />
                    </p>
                `;
                $(newContestant).appendTo('.my-form');
            }

            // Event listener for the Add Contestant button
            $('.add-box').click(function(e){
                e.preventDefault();
                addContestant();
            });

            // Event listener to show the addon input
            $(document).on('click', 'a', function(e) {
                e.preventDefault();
                $(this).siblings('.addon').show();
            });
        });
    </script>
</head>
